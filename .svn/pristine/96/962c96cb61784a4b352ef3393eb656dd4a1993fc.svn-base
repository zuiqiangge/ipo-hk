 ...
 *
 */
public class PgrbJslaxw extends CollectionNews {
	
	@Override
	public void collection() {
		String url = "http://hk.apple.nextmedia.com/realtime/realtimelist/china?page=china";
		try {
			...
			for (Element ele : eles) {
				...
				if (!getNewsService().urlExists(href)) {
					...
					News news = getNewsBean(title, pubDate, SourceInfo.PGRB_JSLAXW, viewNum, desc, context, href);
					//添加到List
					newsList.add(news);
				}
			}
