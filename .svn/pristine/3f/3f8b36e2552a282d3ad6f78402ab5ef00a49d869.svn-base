 ...
 *
 */
public class Xg01Bp extends CollectionNews {

	@Override
	public void collection() {
		String url = "http://www.hk01.com/blogger/01%E5%8D%9A%E8%A9%95";
		String nextUrl = null;
			...
			do {
				...
				if (nextUrl == null || nextUrl.trim().length() == 0) {
					...
					doc = getConnection(nextUrl)
									.header("Referer", "http://www.hk01.com/blogger/01%E5%8D%9A%E8%A9%95")
				...
				for (Element ele : eles) {
					...
					if (!getNewsService().urlExists(href)) {
						...
						News news = getNewsBean(title, pubDate, SourceInfo.XG01_BP, "0", desc, context, href);
						//添加到List
						newsList.add(news);
					}
				}
				if (newsList != null && newsList.size() > 0) {
					...
					nextUrl = "https://www.hk01.com/api/get_articles/39/"+page+"/90/1?TEMPLATES=blogger_article_list";
					page++;
				}else{
