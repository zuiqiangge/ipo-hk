...
public class IsLoginOutInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation actionInvocation) throws Exception {
		...
		// TbXtYhxx tbXtYhxx=(TbXtYhxx) session.get("user");
		// if(tbXtYhxx!=null&&tbXtYhxx.getUserName()!=null&&tbXtYhxx.getUserName().length()>0){
		//		 
		...
		TbXtYhxx tbXtYhxx = (TbXtYhxx) session.get("user");
		if (tbXtYhxx != null && tbXtYhxx.getLoginId() != null
				&& tbXtYhxx.getLoginId().length() > 0) {
			return actionInvocation.invoke();
		} else {
			if (request.getRequestURI().contains("/home_index")) {
				return actionInvocation.invoke();
			} else {
				return "logout";
			}
