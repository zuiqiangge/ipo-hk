 ...
 *
 */
public class NhzbDlxw extends CollectionNews {
	
	@Override
	public void collection() {
		String url = "http://www.scmp.com/news/china";
		...
		try {
			...
			for (Element ele : eles) {
				...
				if (!getNewsService().urlExists(href)) {
					...
					News news = getNewsBean(title, pubDate, SourceInfo.NHZB_DLXW, "0", desc, context, href);
					//添加到List
					newsList.add(news);
				}
			}
