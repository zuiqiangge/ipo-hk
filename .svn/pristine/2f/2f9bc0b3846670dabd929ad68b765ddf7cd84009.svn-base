 ...
 *
 */
public class WhbZgzt extends CollectionNews {
	
	@Override
	public void collection() {
		String url = "http://paper.wenweipo.com/catList-s.php?cat=1000CN&loc=any";
		String nextUrl = null;
		try {
			Document doc = null;
			...
			do {
				...
				for (Element ele : eles) {
					...
					if (!getNewsService().urlExists(href)) {
						...
						News news = getNewsBean(title, pubDate, SourceInfo.WHB_ZGZT, "0", desc, context, href);
						//添加到List
						newsList.add(news);
					}
				}
