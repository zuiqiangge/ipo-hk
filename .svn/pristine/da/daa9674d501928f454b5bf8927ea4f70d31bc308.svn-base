 ...
 *
 */
public class XgxwwGmpl extends CollectionNews {

	@Override
	public void collection() {
		String url = "http://www.hkcna.hk/m/gmpl.html";
		String nextUrl = null;
		int page = 2;
		try {
			do {
				...
				for (Element ele : eles) {
					...
					if (!getNewsService().urlExists(href)) {
						...
						News news = getNewsBean(title, pubDate, SourceInfo.XGXWW_GMPL, "0", desc, context, href);
						newsList.add(news);
					}
				}
				if (newsList != null && newsList.size() > 0) {
					...
					nextUrl = "http://www.hkcna.hk/m/gmpl_" + page + ".html";
