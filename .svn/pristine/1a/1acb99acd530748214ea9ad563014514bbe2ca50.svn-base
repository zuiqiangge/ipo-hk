 ...
 *
 */
public class Xg01Laxw extends CollectionNews {

	@Override
	public void collection() {
		String url = "http://www.hk01.com/section/%E5%85%A9%E5%B2%B8";
		String nextUrl = null;
			...
			do {
				...
				if (nextUrl == null || nextUrl.trim().length() == 0) {
					...
					doc = getConnection(nextUrl)
								.header("Referer", "http://www.hk01.com/section/%E5%85%A9%E5%B2%B8")
				...
				for (Element ele : eles) {
					...
					if (!getNewsService().urlExists(href)) {
						...
						News news = getNewsBean(title, pubDate, SourceInfo.XG01_LAXW, "0", desc, context, href);
						//添加到List
						newsList.add(news);
					}
				}
				if (newsList != null && newsList.size() > 0) {
					...
					nextUrl = "https://www.hk01.com/api/get_articles/3/"+page+"/90?TEMPLATES=blog_listing";
					page++;
				}else{
