 ...
 *
 */
public class YhxgXgxw extends CollectionNews {
	@Test
	public void test(){
	...
	public void collection() {
		String url = "https://hk.news.yahoo.com/hong-kong/archive/1.html";
		String nextUrl = null;
		try {
			do {
				...
				if (nextUrl == null || nextUrl.trim().length() == 0) {
					doc = getConnection(url)
							.header("Referer", "https://hk.news.yahoo.com/hong-kong/")
							.get();
				} else {
				...
				for (Element ele : eles) {
					...
					if (!getNewsService().urlExists(href)) {
						...
						News news = getNewsBean(title, pubDate, SourceInfo.YHXG_XGXW, "0", desc, context, href);
						newsList.add(news);
					}
				}
