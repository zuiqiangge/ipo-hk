 ...
 *
 */
public class PgrbBdxw extends CollectionNews {
	
	@Override
	public void collection() {
		String url = "http://hk.apple.nextmedia.com/video/videolist/"+DateUtil.fmtDt(new Date(), "yyyyMMdd")+"/local/home/0";
		try {
			...
			for (Element ele : eles) {
				...
				if (!getNewsService().urlExists(href)) {
					...
					String viewNum = ele.select(".small_view").text()
							.replace(",", "");
					...
					News news = getNewsBean(title, pubDate, SourceInfo.PGRB_BDXW, viewNum, desc, context, href);
					//添加到List
					newsList.add(news);
				}
			}
