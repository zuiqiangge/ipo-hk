 ...
 *
 */
public class PgrbHome extends CollectionNews {
	
	@Override
	public void collection() {
		String url = "http://hk.apple.nextmedia.com/video/videolist/"+DateUtil.fmtDt(new Date(), "yyyyMMdd")+"/hit/home/0";
		try {
			...
			for (Element ele : eles) {
				...
				if (!getNewsService().urlExists(href)) {
					...
					News news = getNewsBean(title, pubDate, SourceInfo.PGRB_HOME, viewNum, desc, context, href);
					//添加到List
					newsList.add(news);
				}
			}
