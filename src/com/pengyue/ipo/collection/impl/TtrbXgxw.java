 ...
 *
 */
public class TtrbXgxw extends CollectionNews {

	@Override
	public void collection() {
		String url = "http://hd.stheadline.com/news/daily/hk/";
		String nextUrl = null;
		int page = 1;
		try {
			do {
				...
				if (nextUrl == null || nextUrl.trim().length() == 0) {
					...
					doc = getConnection(nextUrl)
							.header("Referer", "http://hd.stheadline.com/news/daily/hk/")
							.get();
				}
				Elements eles = doc.select(".col-xs-12");
				for (Element ele : eles) {
					String pubDate = ele.select(".topic .time").first().ownText().trim();
					//判断新闻是否过期
					if(isOverdue(pubDate,"yyyy-MM-dd")){
						System.out.println("---新闻过期了跳过---");
						break;
					}
					String href = "http://hd.stheadline.com" + ele.select(".topic h4 a").attr("href");
					//判断url是否已存在
					if (!getNewsService().urlExists(href)) {
						Document descDoc = null;
						try {
							descDoc = getConnection(href)
										.get();
						} catch (Exception e) {
						...
						String title = ele.select(".topic h4 a").text();
						String context = descDoc.select("#news-content").text();
						String desc = null;
						...
						News news = getNewsBean(title, pubDate, SourceInfo.TTRB_XGXW, "0", desc, context, href);
						newsList.add(news);
					}
				}
				if (newsList != null && newsList.size() > 0) {
					...
					//下一页地址
					nextUrl = "http://hd.stheadline.com/ajax/getMoreDailyNewsOnList.php?cid=1&page=" + page;
