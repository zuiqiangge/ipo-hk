 ...
 *
 */
public class WhbJszgxw extends CollectionNews {
	
	@Override
	public void collection() {
		String url = "http://news.wenweipo.com/list_news.php?cat=000IN&instantCat=china";
		try {
			...
			for (Element ele : eles) {
				...
				if (!getNewsService().urlExists(href)) {
					...
					News news = getNewsBean(title, pubDate, SourceInfo.WHB_JSZGXW, "0", desc, context, href);
					//添加到List
					newsList.add(news);
				}
			}
