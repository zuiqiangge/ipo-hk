 ...
 *
 */
public class Xg01Gd extends CollectionNews {

	@Override
	public void collection() {
		String url = "http://www.hk01.com/channel/01%E8%A7%80%E9%BB%9E";
		String nextUrl = null;
		int page = 0;
			...
			do {
				...
				if (nextUrl == null || nextUrl.trim().length() == 0) {
					...
					doc = getConnection(nextUrl)
							.header("Referer", "https://www.hk01.com/channel/01%E8%A7%80%E9%BB%9E")
				...
				for (Element ele : eles) {
					...
					if (!getNewsService().urlExists(href)) {
						...
						News news = getNewsBean(title, pubDate, SourceInfo.XG01_GD, "0", desc, context, href);
						//添加到List
						newsList.add(news);
					}
				}
				...
				if (newsList != null && newsList.size() > 0) {
					...
					nextUrl = "https://www.hk01.com/api/get_articles/50/"+page+"/90?TEMPLATES=blog_listing";
					page++;
				}else{
...
//								.header("Referer", "http://www.hk01.com/section/01%E8%A7%80%E9%BB%9E")
//								.header("X-Requested-With", "XMLHttpRequest")
