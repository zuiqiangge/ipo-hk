...
(function($) {
	...
	$.fn.Framer = function(settings) {
		...
		// windowサイズから�??#framerに利用可能なサイズを計算。設�?
		var setBoxSize = function(contents) {
			...
			// #framerContainer のborderとpaddingのサイズを取得する�?�この時点ではwidth、heightとも�?0なはずのなので・・・
			...
			// #framerContainerのpadding、border-sizeとコンテンツのサイズを足した数�?��?�これが#framerのサイズとなるため�?�縮小する場合のベースの値となる
			...
			if(mw > mh) {	// 縦スペースが横スペースより小さい
				if(wh * settings.resizeRatio < targetHeight) {
					...
					if(settings.isAutoResize) {
						...
						// まずは一番親のコンテナとなる#framerのサイズを設定�?�この時�?#framerContainerのpadding.border-sizeを含んだサイ�?
						FRM.box.height(wh * settings.resizeRatio);	// Framerへのpaddingを�?�慮に入れた数�??
		...
		var setContentsSize = function(contents, w, h) {
			...
			if(contentsWidth === 0) {
				// そもそもcontentsのサイズを取得できなかったら、パラメータの�?�そのまま設定す�?
				contents.width(w);
			}
			else {
