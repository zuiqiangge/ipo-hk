...
function operateNews(opt_type,checkedIds, importType, account,timeDomain,starttime,endtime,keywordId,keyword,customFlag1){
//$('.submitBtn').unbind("click"); //移除click
    ...
    if(clickFlag==0){
        clickFlag=1;
        NewsOperate.operateNewsSenior(opt_type,checkedIds, importType, account,timeDomain,starttime,endtime,keywordId,keyword,customFlag1,CallBackSenior);
    }
...
function importNewsSenior(importNewType, folderId, account) {
    ...
    for ( var i = 0; i < elements.length; i++) {
        ...
        if (e.id == "menuCheckId") {
            if (e.checked == true) {
                ...
                checkedIdvalue=checkedIdvalue + e.value+",";
            }
        }
    }
	...
	for(var i=0;i<checkId.length;i++){
		if(document.getElementById("flag_"+checkId[i].split("_")[0])){
			customFlag1 += document.getElementById("flag_"+checkId[i].split("_")[0]).value+',';
		}
	}
    operateNews("insert",checkedIds, importType, account,timeDomain,starttime,endtime,keywordId,keyword,customFlag1);
...
function importNewsSingleSenior(importNewType, folderId, account) {
    ...
    var checkId;
	var customFlag1;
	if(document.getElementById("flag_"+checkedId)){
		customFlag1 = document.getElementById("flag_"+checkedId).value;
	}
    operateNews("insert",checkedIds, importType, account,timeDomain,starttime,endtime,keywordId,keyword,customFlag1);
}
